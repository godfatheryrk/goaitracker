...
Personal venture/goal tracker pairing manual planning with AI-suggested next steps and an expense ledger. Solo MVP, hard deadline **2026-07-04**.

- @context/foundation/prd.md — requirements & success metric
- @context/foundation/tech-stack.md — stack rationale; deploy target = Render
- @context/foundation/shape-notes.md — shaping notes (PL/EN)
...
## Per-user isolation (load-bearing)

NFR(isolation) is a privacy floor — a leak here is an incident regardless of feature correctness. Every per-user table carries an `owner_id` FK to `users.id` with cascade-on-delete, declared as `foreignId('owner_id')->constrained('users')->cascadeOnDelete()` (explicit `'users'` because the column→table inference would otherwise look up `owners`). `owner_id` ≠ `user_id`: the latter is reserved for future membership-pivot semantics. **Authenticated controllers reach per-user data ONLY through a relationship method on the authenticated user** (today: `$request->user()->ventures()` / `$request->user()->expenses()`; the specific method name may evolve in v3+ co-editing, the structural rule does not), never a bare global query (`Venture::find(...)` / `Venture::where(...)`). Route-model binding for owned resources MUST be ownership-scoped. The `auth` middleware is the authentication boundary; public-by-design routes (e.g. a future read-only share-link route) are a separate access category. Full contract incl. v2/v3 future-evolution notes: @docs/reference/contract-surfaces.md (and the S-01 enforcement checklist that the first domain-model slice must satisfy).

## Toolkit-managed block below

The block between `<!-- BEGIN @przeprogramowani/10x-cli -->` and `<!-- END -->` is regenerated by the `@przeprogramowani/10x-cli` toolkit — edits there get overwritten on toolkit updates. Add new project guidance ABOVE the BEGIN marker.

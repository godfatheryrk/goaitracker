...
return [
    ...
    // Default 1.0 (full performance tracing) for the MVP's low traffic; override via
    // SENTRY_TRACES_SAMPLE_RATE (e.g. 0.2) to protect the Render free-tier transaction quota — no code deploy needed.
    ...
    // PINNED false on purpose — NFR(isolation) is the project's privacy floor. With PII off, Sentry sends no
    // email/IP automatically; the SentryContext middleware attaches the numeric user.id ONLY. Do NOT flip this
    // to true (or make it env-driven) without revisiting that privacy decision — it would start exporting email+IP.

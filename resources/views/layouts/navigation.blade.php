    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <div class="flex justify-between items-center h-16 w-full">
            <div class="flex items-center">
                <a href="{{ route('dashboard') }}" class="text-lg font-semibold text-base-content">
                    {{ config('app.name', 'GOAITracker') }}
                </a>
            </div>

            <div class="flex items-center gap-4">
                {{-- Pure-CSS theme toggle: checking the controller sets data-theme=dark, no JS --}}
                <label class="swap swap-rotate" aria-label="Toggle dark theme">
                    <input type="checkbox" class="theme-controller" value="dark" />

                    {{-- sun icon (light theme / unchecked) --}}
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <x-ui.button variant="ghost" class="btn-sm">Sign out</x-ui.button>
                </form>
            </div>
        </div>
    </div>
